...
export function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggle}
      className="relative w-9 h-9 flex items-center justify-center rounded-lg border border-[var(--lp-border)] hover:border-[var(--lp-border-hover)] bg-[var(--lp-glass)] hover:bg-[var(--lp-glass-hover)] transition-all duration-200"
      aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
    >
      {theme === 'dark' ? (
        /* Sun icon */
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="text-[var(--lp-text-secondary)]">
