...
const DEMO_USER_IDS = [
  'user_demo_retail_001',
  'user_demo_retail_002',
  'user_demo_inst_001',
  'user_demo_inst_002',
] as const;
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      activityRows.push({
        partyId: randomElement(PARTY_IDS),
        activityType,
    ...
    for (let i = 0; i < 3; i++) {
      ...
      liqRows.push({
        borrower: randomElement(PARTY_IDS),
    ...
    // -----------------------------------------------------------------------
    // 6b. Demo Auth Users — retail + institutional (MUST be before governance proposals due to FK)
    // -----------------------------------------------------------------------
    ...
    for (const p of proposals) {
      ...
      const proposerUserId = randomElement(DEMO_USER_IDS);
      const proposerParty = randomElement(PARTY_IDS);
      proposalRows.push({
        id: p.id,
        proposalNumber: p.num,
        proposerId: proposerUserId,
        proposerAddress: proposerParty,
        title: p.title,
    ...
    for (let i = 0; i < 6; i++) {
      projectRows.push({
        ...
        ownerPartyId: PARTY_IDS[i % PARTY_IDS.length]!,
    ...
    for (let i = 0; i < 4; i++) {
      ...
      prodBorrowRows.push({
        ...
        borrowerParty: PARTY_IDS[i % PARTY_IDS.length]!,
        projectId: `proj-${String(i + 1).padStart(3, '0')}`,
    ...
    for (let i = 0; i < fracSecurities.length; i++) {
      ...
      fracOfferRows.push({
        ...
        fills: i > 0 ? [{ filledBy: PARTY_IDS[(i + 1) % PARTY_IDS.length], amount: (total - remaining).toFixed(8), filledAt: daysAgo(i).toISOString() }] : [],
        isActive: true,
      });
    }
    ...
    for (let i = 0; i < 100; i++) {
      const pool = randomElement(POOLS);
      const user = randomElement(PARTY_IDS);
