...
// Pool overview is now built dynamically from the pool registry.
...
export function getPoolOverview() {
  return registry.getAllPools().map((pool) => {
    ...
    return {
      poolId: pool.poolId,
      name: `${pool.asset.symbol} Pool`,
      asset: pool.asset.symbol,
      status: pool.isActive ? ('active' as const) : ('paused' as const),
